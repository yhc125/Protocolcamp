...
const EmploymentFactoryContract = require("../artifacts/contracts/EmploymentFactory.sol/EmploymentFactory.json")
...
async function main() {
    ...
    const EmploymentFactory = new ethers.ContractFactory(
        EmploymentFactoryContract.abi, 
        EmploymentFactoryContract.bytecode, 
        deployer)

    const contractDeployed = await EmploymentFactory.deploy();

    //NOTE: you will need this address to run other scripts, so we recommend getting it from the console
    console.log("EmploymentFactory.sol deployed to:", contractDeployed.address)
}
